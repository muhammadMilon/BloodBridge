    ...
    <section className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {rewardBadges.map((badge) => (
          <article
            key={badge.name}
            className="glass border border-rose-100 rounded-3xl p-6 text-center space-y-4 shadow-xl"
          >
            <div
              className={`mx-auto w-20 h-20 rounded-full bg-gradient-to-br ${badge.color} text-white flex items-center justify-center text-2xl font-black`}
            >
              🩸
            </div>
            <h3 className="text-2xl font-bold text-slate-900">{badge.name}</h3>
            <p className="text-sm text-slate-500">{badge.description}</p>
            <p className="text-xs font-semibold text-rose-600 uppercase tracking-[0.5em]">
              {badge.threshold}+ Donations
            </p>
            <button className="w-full px-4 py-3 rounded-xl bg-gradient-to-r from-rose-600 to-red-600 text-white font-semibold">
              Preview Certificate
            </button>
          </article>
        ))}
